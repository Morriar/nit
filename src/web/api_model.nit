...
class APIList
	...
	fun group_mentities(req: HttpRequest, mentities: Array[MEntity]): Array[DocGroup] do
		...
		else if req_group == "intro_redef" then
		...
		else if req_group == "class" then
			grouper = new ClassGroupBuilder(config.view)
		else if req_group == "classdef" then
			grouper = new ClassDefGroupBuilder(config.view)
	...
	redef fun get(req, res) do
		...
		var groups = [new DocGroup.with_mentities(mentities)]
...
class APIEntityInheritance
	...
	redef fun get(req, res) do
		...
		if parents.not_empty then groups.add new DocGroup.with_mentities(parents, "Parents")
		if children.not_empty then groups.add new DocGroup.with_mentities(children, "Children")
		groups = sort_mentities(req, groups)
		groups = limit_mentities(req, groups)
		res.api_json(req, new JsonArray.from(groups))
	end
