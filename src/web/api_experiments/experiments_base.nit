...
class ExpCorpus
	...
	fun get_random(view: ModelView): MEntity do
		var rand = 100.rand
		if rand <= 50 or mentities.is_empty then
			var res = new_random(view)
			if res == null then
				return get_random(view)
			end
			return res
		else
	...
	fun load_random(view: ModelView): MEntity do
		var ref = mentities.rand
		var mentity = view.mentity_by_full_name(ref)
		if not mentity isa MEntity then
			print "Warning: entity `{ref}` not found"
			abort
		end
		return mentity
	end
	...
	fun accept_mentity(mentity: MEntity): Bool do
		...
		if mentity isa MClass and mentity.visibility == private_visibility then return false
