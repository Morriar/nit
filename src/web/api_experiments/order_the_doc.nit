...
class APIExpOrderSession
	...
	private fun create_session(req: HttpRequest, user: User): ExpOrderSession do
		...
		while user.has_ordered(config, full_name) do
		...
		var session = new ExpOrderSession(user, full_name)
		...
		if target isa MPackage then
			...
			for mpackage in config.view.mpackages.to_shuffle do
				if corpus.accept_mentity(mpackage) then
					session.add_to_order mpackage
				end
			end
		end
...
class ExpOrderSession
	...
	# Entity full_name from which we try to order things or null if we order packages
...
redef class User
	...
	fun has_ordered(config: NitwebConfig, full_name: nullable String): Bool do
