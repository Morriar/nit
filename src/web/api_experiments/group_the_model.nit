...
class APIExpGroupSession
	...
	private fun create_session(req: HttpRequest, user: User): ExpGroupSession do
		...
		while user.has_grouped(config, target.full_name) do
			target = corpus.get_random(view)
		end
		config.group_corpus_repo.save(corpus)
		print target.full_name

		var session = new ExpGroupSession(user, target.full_name)
...
redef class User
	...
	fun has_grouped(config: NitwebConfig, full_name: String): Bool do
