...
abstract class CmdList
	...
	# TODO Paginate an array of mentities
	#
	# Returns only a subset of `mentities` depending on the current `page` and the
	# number of elements to return set by `limit`.
	#
	# The `count` parameter can be specified when `mentities` does not contain all
	# the results. For example when the results are already limited from a DB statement.
	fun paginate do
		var results = self.results
		if results == null then return
