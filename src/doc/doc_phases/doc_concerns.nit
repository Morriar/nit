...
redef class MGroupPage
	...
	redef fun build_concerns(v) do
		...
		for mmodule in mentity.mmodules do
			if doc.model.mmodules.has(mmodule) then mmodules.add mmodule
			# collect mclasses
			for mclass in mmodule.intro_mclasses do
				if doc.model.mclasses.has(mclass) then intros.add mclass
			end
			for mclass in mmodule.collect_redef_mclasses(doc.filter) do
				if doc.model.mclasses.has(mclass) then redefs.add mclass
			end
		end
...
redef class MModulePage
	...
	redef fun build_concerns(v) do
		...
		for mclassdef in mentity.mclassdefs do
			if doc.model.collect_mclassdefs(doc.filter).has(mclassdef) then mclassdefs.add mclassdef
		end
		# extract mclasses in mmodule
		for mclassdef in mclassdefs do
			var mclass = mclassdef.mclass
			if doc.model.mclasses.has(mclass) then mclasses.add mclass
		end
		# extract concerns
		var mods = new HashSet[MModule]
		for mclass in mclasses do
			var mod = mclass.intro_mmodule
			if doc.model.mmodules.has(mod) then mods.add mod
		end
	...
	redef fun build_concerns(v) do
		...
		for mclassdef in mclassdefs do
			for mpropdef in mclassdef.mpropdefs do
				if doc.model.collect_mpropdefs(doc.filter).has(mpropdef) then mpropdefs.add mpropdef
			end
		end
		# collect concerns
		var mods = new HashSet[MModule]
