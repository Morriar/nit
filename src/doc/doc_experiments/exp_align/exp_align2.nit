...
class CorpusComposer
	...
	var orig = "clean"
	var out = "composed"
	var dims = ["spans", "codes", "names", "nlp"]

	var corpus_dir = "src/doc/doc_experiments/exp_align/corpus"

	var parser = new ReadmeParser
	var renderer = new RenderReadme

	fun compose do
		var orig_dir = "{corpus_dir}.{orig}"
		var out_dir = "{corpus_dir}.{out}"
		out_dir.mkdir

		for file in orig_dir.files do
			var orig_file = orig_dir / file

			var doc_orig = parser.parse_file(orig_file)

			for dim in dims do
				var dim_file = "{corpus_dir}.{dim}" / file
				var doc_dim = parser.parse_file(dim_file)

				var warns = merge_docs(doc_orig, doc_dim)
				if warns.not_empty then
					print file
					print dim_file
					print warns.join("\n")
				end
			...
			write_doc(doc_orig, out_dir / file)
		end
	end

	fun merge_docs(orig: MdDocument, other: MdDocument): Array[String] do
		var warns = new Array[String]
		for block in orig.children do
			if not block isa MdBlock then continue

			var oblock = other.match_block(block)
			if oblock == null then
				warns.add "Error: cannot find block `{block}`"
				continue
			end

			block.refs.add_all oblock.refs
		end
		return warns
	end

	fun write_doc(orig: MdDocument, file: String) do
		var tpl = new Buffer
		for block in orig.children do
			if not block isa MdBlock then continue

			tpl.append renderer.render(block)
			tpl.append "\n"
			if not block.refs.is_empty then
				tpl.append "\n"
				for ref in block.refs do
					tpl.append "> match: {ref}\n"
				end
				tpl.append "\n"
			end
		end
...
redef class MdBlockQuote
	redef fun render_md(v) do
		if v isa TrimReadme then return
		super
	end
end
	...
	fun match_block(orig: MdBlock): nullable MdBlock do
		for block in blocks do
			if orig isa MdCodeBlock and block isa MdCodeBlock then
				if orig.literal.trim == block.literal.trim then return block
			else
				if orig.raw_text.trim == block.raw_text.trim then return block
			end
		end
		return null
	end
...
redef class MdBlock
	...
	fun parse_entry(lines: Array[String]) do
		for line in lines do
			if line.has(re) then refs.add line.replace(re, "")
		end
	end
end
...
# var input_path = "src/doc/doc_experiments/exp_align/corpus.{input}"
# var output_path = "src/doc/doc_experiments/exp_align/corpus.{output}"
# output_path.mkdir

# var files = input_path.files
# default_comparator.sort(files)

# var parser = new MdParser
# var trimer = new TrimReadme
...
# for file in files do
	...
	# var md = (input_path / file).to_path.read_all
	# var ast = parser.parse(md)
	# var out = trimer.render(ast)
	# out.write_to_file output_path / file

	# sys.system "meld {output_path / file} {input_path / file}"
# end

var comp = new CorpusComposer
comp.compose
