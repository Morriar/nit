...
BUILTIN = ['gettext', 'base64', 'socket', 'codecs', 'traceback', 'symbol', 'json']
...
class PyPackage(PyEntity):
    ...
    def mainmodule(self):
        """ The main module of this package (in a nit sense) """
        if hasattr(self, '_mainmodule'): return self._mainmodule
        if self.has_mainmodule(): return self.submodules[self.name]
        mod = PyModule(self.model, self.name, self)
        mod.doc = 'autogenerated mainmodule'
        mod.imports = dict()
        for name, pkg in self.subpackages.iteritems():
            mod.imports[name] = pkg
        for name, mmod in self.submodules.iteritems():
            if mod == mmod: continue
            mod.imports[name] = mmod
        self._mainmodule = mod
        return mod
...
class PyModule(PyEntity):
    ...
    def _load_imports(self):
        ...
        # print self._list_imports()
        for name in self._list_imports():
            ...
            if name in pcks.keys():
                ...
                # Avoid package loops
                if self in opck.mainmodule().all_imports(): continue
                imports[name] = opck
    ...
    def _list_imports(self):
        """ List imported modules within `import` or `from` clauses """
        ...
        with open(self.path) as f:
            for line in f:
                ...
                if line.startswith('import '):
                    names = line[7:].split(',')
                    for name in names:
                        imports.add(name.strip().split(' ')[0])
                elif line.startswith('from ') and (' import ' in line):
                    name = line[5:].split(' import ')[0]
                    if name == '.': name = self.py_package.name
                    if name == '..': name = self.py_package.parent.name
                    if name.startswith('.'): name = self.py_package.name + '.' + name[1:]
                    if name == 'org.python.core': continue
                    imports.add(name)
        return imports
