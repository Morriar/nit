...
_[gamnit](gamnit::gamnit)_ is modular, different services of the framework are available through different submodules:

* The main entrypoint [`gamnit`](gamnit) provides low-level abstractions over some services of OpenGL ES 2.0, like [textures](gamnit::Texture), [shaders](gamnit::Shader) and programs.
...
* [`flat`](gamnit>flat>) provides an easy to use API for 2D games based on sprites.
  ...
  This [camera](gamnit::Camera) can be moved around in the world by updating the x and y of its `position`,
...
* [`depth`](gamnit>depth>) defines an API for 3D games based on instances of `Actor`.
...
* [`model_parsers`](gamnit>model_parsers>) provides services to read and parse models from the [asset](gamnit>virtual_gamepad>assets>) folder.

* [`network`](gamnit>network>) provides a simple communication framework for multiplayer client/server games.
