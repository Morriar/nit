...
class MdParser
	...
	var inline_parser = new MdInlineParser

	# Block parsers factories
	var block_parser_factories: Collection[MdBlockParserFactory] do
	...
	var active_block_parsers = new Array[MdBlockParser]

	# All active block parsers
	var all_block_parsers = new HashSet[MdBlockParser]

	# Return the active block parser
	#
	# The last entry in the `active_block_parsers` stack.
	fun active_block_parser: MdBlockParser do
	...
	var last_line_blank = new HashMap[MdNode, Bool]
	...
	fun parse(input: String): MdDocument do
		...
		return document_block_parser.block
	end
	...
	#
	# We consider `\r` and `\n`.
	fun find_line_break(input: String, start_index: Int): Int do
