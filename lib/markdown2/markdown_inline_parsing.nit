...
class MdInlineParser
	...
	private var delimiter_processors: Map[Char, MdDelimiterProcessor] is noinit
	private var special_characters: Array[Char] is noinit
	private var delimiter_characters: Array[Char] is noinit
	...
	init do
		delimiter_processors = calculate_delimiter_processors
		delimiter_characters = calculate_delimiter_characters(delimiter_processors.keys)
		special_characters = calculate_special_characters(delimiter_characters)
	end

	# TODO
	fun calculate_delimiter_processors(delimiter_processors: nullable List[MdDelimiterProcessor]): Map[Char, MdDelimiterProcessor] do
		var map = new HashMap[Char, MdDelimiterProcessor]
		add_delimiter_processor(new MdAsteriskDelimiterProcessor, map)
		add_delimiter_processor(new MdUnderscoreDelimiterProcessor, map)
		if delimiter_processors != null then
	...
	# TODO
	fun add_delimiter_processor(delimiter_processor: MdDelimiterProcessor, map: Map[Char, MdDelimiterProcessor]) do
	...
	private fun calculate_delimiter_characters(chars: Collection[Char]): Array[Char] do
	...
	private fun calculate_special_characters(delimiter_characters: Array[Char]): Array[Char] do
		var chars = ['\n', '`', '[', ']', '\\', '!', '<', '&']
		chars.add_all delimiter_characters
		return chars
	end
	...
	fun append_text(text: String, begin_index, end_index: nullable Int): MdText do
		var node: MdText
	...
	fun append_node(node: MdNode) do block.append_child(node)
	...
	private fun parse_inline: Bool do
		...
		if c == '\n' then
			...
			var is_delimiter = delimiter_characters.has(c)
			if is_delimiter then
				var delimiter_processor = delimiter_processors[c]
				res = parse_delimiters(delimiter_processor, c)
			else
				res = parse_string
			end
		end
	...
	fun parse_close_bracket: Bool do
		advance 1
		var start_index = index
		var start_column = column
	...
	private fun parse_link_destination: Couple[nullable String, Bool] do
		...
		return new Couple[nullable String, Bool](buffer.to_s, false)
	end
	...
	private fun process_delimiters(stack_bottom: nullable MdDelimiter) do
		...
		while closer != null do
			...
			if not delimiter_processors.has_key(delimiter_char) then
				closer = closer.next
				continue
			end

			var delimiter_processor = delimiter_processors[delimiter_char]
